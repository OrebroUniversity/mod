...
| Footprint test | One circumscribed circle for every shape: circle → its radius; rectangle → `sqrt((L/2)² + (W/2)²)`. Tested as the rasterized disc of pixels around the pose (offset list precomputed once per map). No distance transform, no polygon test; sophistication later if needed. |
...
- A rasterized disc for a 0.4 m radius at 0.05 m is about 800 pixel lookups per pose; on the 34k-node ATC Dijkstra grid that is ~27M lookups at setup, well under a second. If a much finer map appears, the disc offset list is where a distance transform would slot in.
